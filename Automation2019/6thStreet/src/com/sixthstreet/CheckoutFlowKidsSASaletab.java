...
public class CheckoutFlowKidsSASaletab {
	...
	public static void saudiarabiaKidSaleTab() throws Exception {
		try {
			...
			FirstClass.takeSnapShot(driver, ".//ScreenShots//catchCheckoutFlowKWWomenSalespic.png");
			driver.quit();
		}
	...
	public static void verifyPaymentPage() {
		try {			
			if(driver.findElement(By.xpath("//div[@class='step-title active'][contains(text(),'3. Payment')]")).isDisplayed())
			{
				...
				FirstClass.takeSnapShot(driver, ".//ScreenShots//SuccessfullyVerifiedCheckoutFlowKWWomenSaletab.png");

			}else{
				System.out.println("Error Coming while verifed Kuwait Checkout flow For Kids Sale Tab");
				FirstClass.takeSnapShot(driver, ".//ScreenShots//ErrorComingInCheckoutFlowKWWomenSaletab.png");
			}			
		}
		catch(Exception e){
			e.getMessage();			
