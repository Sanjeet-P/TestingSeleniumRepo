...
public class CheckoutFlowUAE {
	...
	public static void dsvfinal() throws Exception {
		try {
		openingBrowser();
		verifyURL();
		driver.manage().timeouts().implicitlyWait(30, TimeUnit.SECONDS);
		...
		FirstClass.takeSnapShot(driver, ".//ScreenShots//catchCheckoutFlowUAE.png");
		driver.quit();
	}}
	...
	public static void verifyPaymentPage() {
		try {			
			if(driver.findElement(By.xpath("//div[@class='step-title active'][contains(text(),'3. Payment')]")).isDisplayed())
			{
				...
				FirstClass.takeSnapShot(driver, ".//ScreenShots//SuccessfullyVerifiedCheckoutFlowAE.png");
				
			}else{
				System.out.println("Error Coming while verifed Saudi Arabia Checkout flow For New In Tab");
				FirstClass.takeSnapShot(driver, ".//ScreenShots//ErrorComingInCheckoutFlowAE.png");
			}			
		}
		catch(Exception e){
			e.getMessage();			
